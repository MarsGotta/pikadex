<template>
  <section class="search-list">
    <p>
      <strong>{{ pokemons.length }}</strong> Pokemons for you to choose your
      favorite:
    </p>
    <mars-search
      class="list-search"
      placeholder="Find your pokemon"
      @search="(search) => handlePagination({ search, page: 1 })"
    ></mars-search>
    <mars-list
      :items="pokemonsFiltered"
      :on-pagination="(page) => handlePagination({ page: Number(page) })"
